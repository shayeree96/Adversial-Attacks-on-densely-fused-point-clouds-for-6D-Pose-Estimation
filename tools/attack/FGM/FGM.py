...
class FGM:
    ...
    def get_gradient(self, img, data, choose, idx, model_points, target, w, refine_start, normalize=True):  # [B, 3, K]
    # def get_gradient(self, data, target, normalize=True):
        ...
        # forward pass
        pred_r, pred_t, pred_c, emb = self.model(img, data, choose, idx)
        loss, dis, new_points, new_target = self.adv_func(pred_r, pred_t, pred_c, target, model_points, idx, data, w, refine_start)
        ...
        return grad
    # img, points, choose, idx, target_label
    # def attack(self, data, target):
    def attack(self, img, data, choose, idx, model_points, target, w, refine_start):
        """One step FGM attack.
        ...
        pc = data.clone().detach().contiguous()
        target = target.float().cuda()

        # gradient
        normalized_grad = self.get_gradient(img, pc, choose, idx, model_points, target, w, refine_start)  # [B, 3, K]

        perturbation = normalized_grad * self.budget

        with torch.no_grad():
            perturbation = perturbation.contiguous()
